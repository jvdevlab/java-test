...
public class ProducerConsumerPattern {
    ...
    public void producerConsumerPattern() throws InterruptedException {
        ...
        assertEquals(Thread.State.WAITING, producer.getState()); // still needs to add 5

        // tries to consume 15 messages.
        // 5 on the queue + another 5 producer will add + blocks waiting on another 5
        consumer.start();
        Thread.sleep(500);
        assertEquals(5, queue.remainingCapacity());
        assertEquals(Thread.State.WAITING, consumer.getState()); // wait for another 5
